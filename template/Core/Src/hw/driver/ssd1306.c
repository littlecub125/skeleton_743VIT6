...
//-- Functions
//
bool ssd1306WriteCmd(uint8_t cmd);
bool ssd1306WriteData(uint8_t data);
void ssd1306_I2C_WriteMulti(uint8_t reg, uint8_t *data,
    uint16_t count);
...
void SSD1306_SetInitScreen(void);
...
{
  uint8_t dt[2];
  dt[0] = SSD1306_REG_COMMAND;
  dt[1] = cmd;

  i2cWriteBytes(HW_I2C_CH_DISPLAY_SSD_1306, dt, 2);
  return true;
}

void SSD1306_ScrollRight(uint8_t start_row, uint8_t end_row)
{
  ssd1306WriteCmd(SSD1306_RIGHT_HORIZONTAL_SCROLL);  // send 0x26
  ssd1306WriteCmd(0x00);  // send dummy
  ssd1306WriteCmd(start_row);  // start page address
  ssd1306WriteCmd(0X00);  // time interval 5 frames
  ssd1306WriteCmd(end_row);  // end page address
  ssd1306WriteCmd(0X00);
  ssd1306WriteCmd(0XFF);
  ssd1306WriteCmd(SSD1306_ACTIVATE_SCROLL); // start scroll
}

void SSD1306_ScrollLeft(uint8_t start_row, uint8_t end_row)
{
  ssd1306WriteCmd(SSD1306_LEFT_HORIZONTAL_SCROLL);  // send 0x26
  ssd1306WriteCmd(0x00);  // send dummy
  ssd1306WriteCmd(start_row);  // start page address
  ssd1306WriteCmd(0X00);  // time interval 5 frames
  ssd1306WriteCmd(end_row);  // end page address
...
{
  ssd1306WriteCmd(SSD1306_SET_VERTICAL_SCROLL_AREA);  // sect the area
  ssd1306WriteCmd(0x00);   // write dummy
  ssd1306WriteCmd(SSD1306_HEIGHT);

  ssd1306WriteCmd(SSD1306_VERTICAL_AND_RIGHT_HORIZONTAL_SCROLL);
  ssd1306WriteCmd(0x00);
  ssd1306WriteCmd(start_row);
  ssd1306WriteCmd(0X00);
  ssd1306WriteCmd(end_row);
...
void SSD1306_SetInitScreen(void)
{
  ...
  SSD1306_Puts("  Completed", &Font_11x18, 1);
...
void SSD1306_UpdateScreen(void)
{
  ...
  for (m = 0; m < 8; m++)
  {
    ssd1306WriteCmd(0xB0 + m);
    ssd1306WriteCmd(0x00);
    ssd1306WriteCmd(0x10);

    /* Write multi data */
    ssd1306_I2C_WriteMulti(SSD1306_REG_DATA,
...
static void cliSsd1306(int argc, char *argv[])
{
  ...
  if (argc == 2)
  {
    if (cliCheck(argv[1], "addr"))
    {
      cliPrintf("0x%x", SSD1306_I2C_ADDR);
      ret = true;
    }
  }
  else if (argc == 3)
  {
    if (cliCheck(argv[1], "write"))
    {
      SSD1306_Fill(SSD1306_COLOR_BLACK);
      SSD1306_GotoXY(0, 0);
      SSD1306_Puts(argv[2], &Font_11x18, 1);
      SSD1306_UpdateScreen();
      ret = true;
    }
  }
    ...
    uint16_t count)
{
  ...
  i2cWriteBytes(HW_I2C_CH_DISPLAY_SSD_1306, dt, count + 1);
  return;
}
